  ...
  while (in_buffer->ReadIndex() < in_buffer->WriteIndex()) {
    ...
    if (parse_success) {
      ...
      if (msg_id_index >= end_index) {
        ...
        ERRORLOG("parse error, msg_id_index[%d] >= end_index[%d]", msg_id_index,
                 end_index);
        continue;
      }
      message->msg_id_ = getInt32FromNetByte(&tmp[msg_id_index]);
      DEBUGLOG("parse msg_id_=%d", message->msg_id_);
...
const char *TinyPBCoder::encodeTinyPB(TinyPBProtocol::s_ptr message, int &len) {
  ...
  INFOLOG("msg_id = %d", message->msg_id_);
  ...
  *tmp = TinyPBProtocol::PB_END;
  ...
  message->parse_success_ = true;
  len = pk_len;
  DEBUGLOG("msg_id = %d, encode message success", message->msg_id_);
  return buffer;
}
} // namespace rocket
